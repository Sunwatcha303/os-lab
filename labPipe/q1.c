...
int main() {
    ...
    if (pid == 0) { //child
        ...
        while ((nread = read(pfd[0], buf/* q1.2 */, SIZE)) != 0)
            if (nread > 11)
        ...
        close(pfd[0]);/* q1.5 */  //tidy unused end
        // strcpy(inbuf,"4321"); //equiv "4321\0"
        sprintf(inbuf, "%ld", 123456789012);
        // newer function - less side effect
        write(pfd[1],inbuf,strlen(inbuf)+1);/* q1.3 */ // +1 for padded \0
        close(pfd[1]);/* q1.4 */ 
        wait(NULL);/* q1.5 */; //wait for child to complete
    }
    return 0;
}